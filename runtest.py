...
for img_id, style_name in enumerate(Style):
    ...
    os.system(
        "python Segmentation/segmentation.py --img Input/style/%s.png --img_content Input/style/%s_glyph.png"
        % (style_name, style_name)
    )
    os.system(
        "python Segmentation/crf_post_process.py --img Input/style/%s.png"
        % (style_name)
    )
    print("Successfully generate mask_final.jpg and mask_ori.jpg.")

    if not (
        os.path.exists("temp/mask_final.jpg") and os.path.exists("temp/mask_ori.jpg")
    ):
    ...
    if os.path.exists("cache/%s_netG.pth" % (style_name)):
        ...
        os.system(
            "python FineTuning/train.py --gpu %d --batchSize %d --niter %d\
                        --netf pre-trained \
                        --style_name %s \
    ...
    if os.path.exists(
        "basal-text-effect-transfer/%s.png" % (style_name + "_" + content_name)
    ):
        print(
            "Exists %s.png. Skip basal text effect transfer."
            % (style_name + "_" + content_name)
        )
    else:
        os.system(
            "python FineTuning/generate.py --gpu %d\
