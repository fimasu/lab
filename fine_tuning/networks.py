...
def define_G(input_nc, output_nc):
    ...
    netG = UnetGenerator(input_nc, output_nc, 8, 64, norm_layer=norm_layer, use_dropout=False)  # type: ignore
    return netG
...
def define_D(input_nc):
    norm_layer = get_norm_layer(norm_type="batch")
    netD = NLayerDiscriminator(input_nc, 64, n_layers=3, norm_layer=norm_layer, use_sigmoid=False)  # type: ignore
    return netD
...
class UnetGenerator(nn.Module):
    ...
    def forward(self, x, size):
        assert size in {32, 64, 128, 256}, "only supports resolution: 32, 64, 128, 256."
...
class DownBlock(nn.Module):
    def __init__(
        ...
        if isinstance(norm_layer, functools.partial):
            use_bias = norm_layer.func == nn.InstanceNorm2d  # type: ignore  # type: ignore
        else:
...
class UpBlock(nn.Module):
    def __init__(
        ...
        if isinstance(norm_layer, functools.partial):
            use_bias = norm_layer.func == nn.InstanceNorm2d  # type: ignore
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(
        ...
        self.model = nn.Sequential(*model)  # type: ignore  # type: ignore
